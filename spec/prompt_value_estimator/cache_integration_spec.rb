  ...
  describe 'caching in get_related_prompts' do
    before do
      allow(mock_serpstat_client).to receive(:get_related_keywords).and_return([
        { keyword: 'related1', search_volume: 500, cpc: 1.50, competition: 0.60, source: 'serpstat' },
        { keyword: 'related2', search_volume: 300, cpc: 0.80, competition: 0.40, source: 'serpstat' }
      ])
    end
  ...
  describe 'caching in get_keyword_suggestions' do
    before do
      allow(mock_serpstat_client).to receive(:get_keyword_suggestions).and_return([
        { keyword: 'suggestion1', search_volume: 800, cpc: 2.00, competition: 0.70, source: 'serpstat' },
        { keyword: 'suggestion2', search_volume: 600, cpc: 1.20, competition: 0.50, source: 'serpstat' }
      ])
    end
